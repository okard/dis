...
# spec fields:
SPEC_DESC = 'desc'
# spec for compilation:
SPEC_COMP_CMD = 'compile-cmd'
SPEC_COMP_RESULT = 'compile-result'
SPEC_COMP_ENV = 'compile-env'

SPEC_COMP_STDOUT = 'compile-stdout'
SPEC_COMP_STDERR = 'compile-stderr'

# spec for running test file
SPECRUNCMD = 'run-cmd'
SPECRUNRESULT = 'run-result'
SPEC_RUNENV = 'run-env'

# run output specs
SPEC_RUN_STDOUT = 'run-stdout'
SPEC_RUN_STDERR = 'run-stderr'
...
def main(argv=None):
	...
	#TODO command line parsing run a folder of test files
	...
	for testfile in testfiles:
		...
		print(" {1} '{0}'".format(spec.get(SPEC_DESC, ""), normalFile),)
...
def compileTest(spec, testfile):
	...
	try:
		...
		cmd_str = spec[SPEC_COMP_CMD].format(disc=DISC, srcfile=testfile, outname=os.path.join(BINDIR, binname))
		...
		if SPEC_COMP_RESULT in spec:
			
			color = FAIL
			expected = int(spec[SPEC_COMP_RESULT])
			...
			write(WARNING, "Missing Spec (Result: {0}, Spec: {1})".format(ret, SPEC_COMP_RESULT));
...
def runTest(spec, testExec):
	...
	if testExec == None or not os.path.isfile(testExec):
		color = FAIL
		if int(spec.get(SPEC_COMP_RESULT, "0")) != 0:
			color = OKGREEN
